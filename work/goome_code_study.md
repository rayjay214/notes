...
由于有很多动态语言和很多种Web服务器，他们彼此之间互不兼容，给程序员造成了很大的麻烦。那么，CGI应运而僧。CGI的定义是统一网关接口。从此WebServer收到后台动态交互请求就直接发给CGI,CGI发给动态语言，动态语言把结果发回给CGI，CGI再发回给WebServer，后面的事情你都清楚了。。。。
